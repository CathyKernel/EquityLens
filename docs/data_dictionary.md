...
|---|---|---|
| `ticker` | TEXT | Ticker symbol, uppercase (e.g. `AAPL`, `MSFT`). 21 constituents + `SPY` benchmark. |
...
| `ticker_sector` | ticker | Static ticker → GICS sector mapping (8 sectors). |
...
`sector_<Name>` — one-hot GICS sector indicators (7 columns + dropped
baseline, from the static 8-sector map).
...
| `outputs/report.html` | Self-contained HTML report with all figures embedded (base64). |
